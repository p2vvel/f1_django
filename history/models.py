...
class Drivers(models.Model):
    ...
    def __str__(self):
        if self.code or self.number:
            return "[{}{}{}]".format("%s" % self.code if self.code else "", " " if self.code and self.number else "", "%s" % self.number if self.number else "") +\
...
class Constructors(models.Model):
    id = models.AutoField(
        db_column='constructorId', primary_key=True)
    nickname = models.CharField(
        db_column='constructorRef', max_length=255)
...
class Constructorresults(models.Model):
    id = models.AutoField(
        db_column='constructorResultsId', primary_key=True)
...
class Constructorstandings(models.Model):
    id = models.AutoField(
        db_column='constructorStandingsId', primary_key=True)
    race = models.ForeignKey('Races', models.DO_NOTHING, db_column='raceId')
    constructor = models.ForeignKey(
        'Constructors', models.DO_NOTHING, db_column='constructorId')
...
class Driverstandings(models.Model):
    id = models.AutoField(
        db_column='driverStandingsId', primary_key=True)
    race = models.ForeignKey('Races', models.DO_NOTHING, db_column='raceId')
    driver = models.ForeignKey(
        'Drivers', models.DO_NOTHING, db_column='driverId')
    points = models.FloatField()
    position = models.IntegerField(blank=True, null=True)
    position_info = models.CharField(
        db_column='positionText', max_length=255, blank=True, null=True)
...
class Races(models.Model):
    ...
    circuit = models.ForeignKey(
        Circuits, models.DO_NOTHING, db_column='circuitId')
    name = models.CharField(max_length=255)
...
class Results(models.Model):
    ...
    fastest_lap = models.IntegerField(
        db_column='fastestLap', blank=True, null=True)
